...
{    
    ...
    XCTAssertTrue( 0 != isThreadSafe_, @"Apple compiles sqlite with mutexes enabled" );
    XCTAssertTrue( SQLITE_CONFIG_MULTITHREAD  == isThreadSafe_, @"Apple builds in multithreaded mode" );
}

-(void)testCanSwitchTo
{
    int errorCode_ = sqlite3_config( SQLITE_CONFIG_SERIALIZED );
    XCTAssertTrue( errorCode_ == SQLITE_OK, @"SQLite should be able to switch in Serialized mode" );
}
...
-(void)testOpenInSerializedMode
{
    ...
    XCTAssertTrue( error_ == SQLITE_OK, @"SQLite can open DB in serialized mode" );
}
